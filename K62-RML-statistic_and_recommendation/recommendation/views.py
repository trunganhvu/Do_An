...
def create_train_data_file(request, major_id=None):
    ...
    if major_id is not None and request.method == 'POST':
        ...
        file_name = str(major_id) + "_train_data.csv"
        train_file, created = mainModel.TrainData.objects.update_or_create(major_id=major_id,
                                                                           defaults={'dataPath': file_name})
...
def predict_grade(student, semester=None, course_id_list: list = None, fitting: bool = True, save_history: bool = True):
    ...
    if course_id_list is None or len(course_id_list) < 1:
        ...
        course_id_list = mainModel.Major_Course.objects.filter(major_id=student.major_id, semesterRecommended=semester_rank
...
def student_predict_loss(student):
    ...
    num = len(loss)
    loss = np.array(loss)
    mea = np.abs(loss).mean()
    rmse = loss * loss
    rmse = rmse.mean()
    result = {
        'total_predict': num,
        'mea': 0 if np.isnan(mea) else mea,
...
def model_predict_loss():
    # return student_predict_loss(mainModel.Profiles.objects.get(pk=33))
    ...
    for stu_pre in student_predicts:
        try:
            ...
            result['total_predict'] += pre_loss['totalcurrent_semester_predict']
            result['mea'] += pre_loss['total_predict'] * pre_loss['mea']
            result['rmse'] += pre_loss['total_predict'] * pre_loss['rmse']
        except Exception as e:
            print(e)
    print("model_predict_loss:", pre_loss)
    if pre_loss['total_predict'] != 0:
        result['mea'] /= pre_loss['total_predict']
...
def predict_grade_generation(request, major_id):
    ...
    for generation_id in list_generation_id:
        ...
        course_id_list = mainModel.Major_Course.objects.filter(major_id=major_id, semesterRecommended=semester_rank
                                                               ).values_list('course_id', flat=True)
...
def similar_neighborCR(student, course_can_learn, k=5, sim='cosine'):
    ...
    # student_grade = student_grade[list(set(user_score.columns) & set(data.columns))]

    # get recommend course in next semester

    similarity_df = sim(data.loc[:, student_grade.columns], student_grade)
